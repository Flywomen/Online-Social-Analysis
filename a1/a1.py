...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        components = partition_girvan_newman(cgraph, depth)
        score.append((depth, norm_cut(components[0], components[1], cgraph)))
    return score
...
def make_training_graph(graph, test_node, n):
    ...
    """
    cgraph = graph.copy()
    neighbornodes = sorted(cgraph.neighbors(test_node),key = lambda x:x[0])
    for i in range(0,n):
        cgraph.remove_edge(*(test_node, neighbornodes[i]))
    return cgraph  
...
def jaccard(graph, node, k):
    ...
    [(('D', 'E'), 0.5), (('D', 'A'), 0.0)]
    """
    neighbornodes = set(graph.neighbors(node))
    scores = []
    
    scoresnode = [x for x in graph.nodes() if x not in neighbornodes and x != node]
    
    for n in scoresnode:
        neighbors = set(graph.neighbors(n))
        #if node != n and not graph.has_edge(node, n):
            
        score = ((node, n), len(neighbornodes & neighbors) / len(neighbornodes | neighbors))
        scores.append(score)
    lists=sorted(scores, key=lambda x : (-x[1], x[0][1]))[:k]
    lists.pop(0)
    return lists[:k]
...
def path_score(graph, root, k, beta):
    ...
    scores = []
    cgraph = graph.copy()
    node2distances, node2num_paths, node2parents= bfs(graph, root, max_depth=max(graph.degree(graph.nodes()).values()))
    for node in graph.nodes():
