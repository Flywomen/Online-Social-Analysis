...
def friend_overlap(users):
    ...
    for i in range(0, len(users)):
        for j in range(0, len(users)):
            if (i < j):
                name1 = users[i]['screen_name']
                name2 = users[j]['screen_name']
                overlap = [m for m in users[i]['friends'] if m in users[j]['friends']]
                commom_friend.append((name1, name2, len(overlap))) 
                
    return sorted(commom_friend, key=lambda x: (-x[2], x[0], x[1]))
...
def draw_network(graph, users, filename):
    ...
    label = {}
    for user in users:
        label[user['screen_name']] = user['screen_name']
