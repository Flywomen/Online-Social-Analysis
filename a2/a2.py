...
def token_pair_features(tokens, feats, k=3):
    ...
    if k >= 2 :
        for i in range(len(tokens)-k+1):
            com = combinations(tokens[i:i+k],2)
            for j in com:
                pair = 'token_pair='+j[0]+'__'+j[1]
                if pair in feats:
                    feats[pair] += 1
                else:
                    feats[pair] = 1    
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for token in tokens_list:
        f = featurize(token, feature_fns)
        features_dict[count] = f
        for i in f:
            g, h = i
            c.update([g])
        count =count+1
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    for training, testing in cv.split(X):
        ...
        predictions = clf.predict(X[testing])
        accy=accuracy_score(labels[testing], predictions)
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    return sorted(dict_list,key=lambda x:(-x['accuracy'],-x['min_freq']))
...
def mean_accuracy_per_setting(results):
    ...
    for k, v in mean_acc.items():
        avg = np.mean(v)
        m.append((avg,k))     
    return sorted(m, key=lambda k:k[0])[::-1]
    #pass
...
def fit_best_classifier(docs, labels, best_result):
    ...
    """
    ###TODO
    keep_internal_punct = best_result['punct'] 
    tokens_list = list()
    for doc in docs:
        tokens_list.append(tokenize(doc, keep_internal_punct))
...
def top_coefs(clf, label, n, vocab):
    ...
    cl = clf.coef_[0]
    for feat in vocab:
            result.append((feat,cl[vocab[feat]]))          
    if label == 1:
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    x_prediction = clf.predict_proba(X_test)
    wrong_prediction = list()
    for i in range(len(predictions)):
        if(predictions[i] != test_labels[i]):
            wrong_prediction.append((x_prediction[i][predictions[i]],i,predictions[i],test_labels[i]))
    sort_prediction = sorted(wrong_prediction, key=lambda x:-x[0])
    for tup in sort_prediction[:n]:
